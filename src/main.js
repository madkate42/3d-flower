...
const BUTTONS = [
  ...
  { id: 'btn3', position: [0.5, 1.8, 0.5], url: 'https://example.com/3', label: 'Contact' },
  { id: 'btn4', position: [-0.3, 0, 1.65], url: 'https://github.com/madkate42', label: 'GitHub' },
  { id: 'btn5', position: [-0.35, 0.55, 1.25], url: 'ttps://example.com/4', label: 'Resume' },
];
...
function onMouseMove(event) {
  ...
  if (intersects.length > 0) {
    ...
    // Highlight effect
    hoveredButton.material.color.multiplyScalar(1.5);
    hoveredButton.scale.setScalar(1.2);

    // Show tooltip
    tooltip.textContent = hoveredButton.userData.label;
...
function onClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    console.log('Clicked:', clickedButton.userData.id, clickedButton.userData.label);

    if (clickedButton.userData.url) {
      window.open(clickedButton.userData.url, '_blank');
    }
  }
}
